...
with tab1:
    ...
    if not df_filtered.empty and "城市" in df_filtered.columns:
        ...
        ca,cb = st.columns(2)
        with ca:
            fig = px.bar(cg,x="城市",y="客资总数",title="各城市客资量",color="城市",color_discrete_sequence=COLORS,category_orders={"城市":CITIES})
            st.plotly_chart(make_chart(fig),use_container_width=True)
        with cb:
            fig = px.bar(cg,x="城市",y="收销总量",title="各城市成交量",color="城市",color_discrete_sequence=COLORS,category_orders={"城市":CITIES})
            st.plotly_chart(make_chart(fig),use_container_width=True)
        cc,cd = st.columns(2)
        with cc:
            fig = px.bar(cg,x="城市",y="到店总量",title="各城市到店量",color="城市",color_discrete_sequence=COLORS,category_orders={"城市":CITIES})
            st.plotly_chart(make_chart(fig),use_container_width=True)
        with cd:
            fig = px.bar(cg,x="城市",y="到店率%",title="各城市到店率(%)",color="城市",color_discrete_sequence=COLORS,category_orders={"城市":CITIES})
            st.plotly_chart(make_chart(fig),use_container_width=True)
        ce,cf = st.columns(2)
        with ce:
            fig = px.bar(cg,x="城市",y="客资成本",title="各城市客资成本",color="城市",color_discrete_sequence=COLORS,category_orders={"城市":CITIES})
            st.plotly_chart(make_chart(fig),use_container_width=True)
        with cf:
            fig = px.bar(cg,x="城市",y="成交成本",title="各城市成交成本",color="城市",color_discrete_sequence=COLORS,category_orders={"城市":CITIES})
            st.plotly_chart(make_chart(fig),use_container_width=True)
