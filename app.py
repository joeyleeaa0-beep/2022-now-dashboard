...
c1.metric("总花费", f"¥{total_spend:,.0f}")
c2.metric("总客资量", f"{int(total_keizi):,}")
c3.metric("到店总量", f"{int(total_daodian):,}")
c4.metric("总成交量", f"{int(total_chengjiao):,}")
c5,c6,c7,c8 = st.columns(4)
c5.metric("销售量", f"{int(total_xiaoshou):,}")
c6.metric("收购量", f"{int(total_shougou):,}")
c7.metric("客资成本", f"¥{keizi_cost:.2f}")
c8.metric("成交成本", f"¥{chengjiao_cost:.2f}")
c9,c10,_,_ = st.columns(4)
c9.metric("到店率", f"{daodian_rate:.2f}%")
c10.metric("成交率", f"{chengjiao_rate:.2f}%")
...
with tab1:
    ...
    if not df_filtered.empty and "城市" in df_filtered.columns:
        ...
        ca,cb = st.columns(2)
        with ca:
            fig = px.bar(cg,x="城市",y="客资总数",title="各城市客资量",color="城市",color_discrete_sequence=COLORS)
            st.plotly_chart(make_chart(fig),use_container_width=True)
        with cb:
            fig = px.bar(cg,x="城市",y="收销总量",title="各城市成交量",color="城市",color_discrete_sequence=COLORS)
            st.plotly_chart(make_chart(fig),use_container_width=True)
        cc,cd = st.columns(2)
        with cc:
            fig = px.bar(cg,x="城市",y="到店总量",title="各城市到店量",color="城市",color_discrete_sequence=COLORS)
            st.plotly_chart(make_chart(fig),use_container_width=True)
        with cd:
            fig = px.bar(cg,x="城市",y="到店率%",title="各城市到店率(%)",color="城市",color_discrete_sequence=COLORS)
            st.plotly_chart(make_chart(fig),use_container_width=True)
        ce,cf = st.columns(2)
        with ce:
            fig = px.bar(cg,x="城市",y="客资成本",title="各城市客资成本",color="城市",color_discrete_sequence=COLORS)
            st.plotly_chart(make_chart(fig),use_container_width=True)
        with cf:
            fig = px.bar(cg,x="城市",y="成交成本",title="各城市成交成本",color="城市",color_discrete_sequence=COLORS)
            st.plotly_chart(make_chart(fig),use_container_width=True)
...
with tab3:
    ...
    if not df_trend.empty and "月份" in df_trend.columns:
        ...
        fig = px.line(tm,x="月份",y="客资总数",color="年份",title="各年度客资量月度趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig),use_container_width=True)
        fig2 = px.line(tm,x="月份",y="收销总量",color="年份",title="各年度成交量月度趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig2),use_container_width=True)
        fig3 = px.line(tm,x="月份",y="到店率%",color="年份",title="各年度到店率月度趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig3),use_container_width=True)
        fig4 = px.line(tm,x="月份",y="客资成本",color="年份",title="各年度客资成本月度趋势",markers=True,color_discrete_sequence=COLORS)
...
with tab4:
    ...
    if available_spend:
        ...
        if not spend_df.empty:
            sa,sb = st.columns(2)
            with sa:
                fig = px.pie(spend_df,names="渠道",values="花费",title="各渠道花费占比",color_discrete_sequence=COLORS)
                fig.update_traces(textposition='inside', textinfo='percent+label')
                st.plotly_chart(make_chart(fig),use_container_width=True)
            with sb:
                fig = px.bar(spend_df,x="渠道",y="花费",title="各渠道花费对比",color="渠道",color_discrete_sequence=COLORS)
                st.plotly_chart(make_chart(fig),use_container_width=True)
    ...
    if not df_spend_trend.empty and "年份" in df_spend_trend.columns:
        ...
        fig = px.line(sg,x="年份",y="总花费",title="各年度总花费趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig),use_container_width=True)
