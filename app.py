...
def clean_df():
    ...
    if "月份" in df.columns:
        ...
        # 补"月"字（有些可能只是数字）
        df["月份"] = df["月份"].apply(
            lambda x: x + "月" if x.isdigit() else x
        )
    # 数值列转换
    skip_cols = ["城市", "年份", "月份"]
...
with tab3:
    ...
    if not df_trend.empty and "月份" in df_trend.columns:
        ...
        fig = px.line(tm,x="月份",y="客资总数",color="年份",
                      title="各年度客资量月度趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig),use_container_width=True)
        fig2 = px.line(tm,x="月份",y="收销总量",color="年份",
                       title="各年度成交量月度趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig2),use_container_width=True)
        fig3 = px.line(tm,x="月份",y="到店率%",color="年份",
                       title="各年度到店率月度趋势",markers=True,color_discrete_sequence=COLORS)
        st.plotly_chart(make_chart(fig3),use_container_width=True)
        fig4 = px.line(tm,x="月份",y="客资成本",color="年份",
                       title="各年度客资成本月度趋势",markers=True,color_discrete_sequence=COLORS)
